 ...
 */
class MultiPassAgent extends MinimalReactAgent_1.MinimalReactAgent {
     ...
     */
    async analyze(error) {
        const perf = new PerformanceTracker_1.PerformanceTracker();
     ...
     */
    async generateHypotheses(error) {
        const hypotheses = [];
     ...
     */
    buildDiversityPrompt(error, existingHypotheses, index) {
        const existingSummary = existingHypotheses.length > 0
            ? `\n\n**EXISTING HYPOTHESES (generate a DIFFERENT one):**\n${existingHypotheses.map((h, i) => `${i + 1}. ${h.rootCause.substring(0, 100)}...`).join('\n')}`
            : '';
...
OUTPUT ONLY VALID JSON:`;
     ...
     */
    parseHypothesisResponse(response, index) {
        try {
     ...
     */
    async validateHypotheses(error, hypotheses) {
     ...
     */
    extractEvidence(result, hypothesis) {
        const evidence = [];
     ...
     */
    detectContradictions(result, hypothesis) {
     ...
     */
    calculateEvidenceScore(evidence, contradictions) {
     ...
     */
    async buildConsensus(hypotheses) {
     ...
     */
    buildConsensusPrompt(hypotheses) {
        const hypothesesText = hypotheses.map((h, i) => `**Hypothesis ${i + 1}** (confidence: ${(h.confidence * 100).toFixed(0)}%):\n${h.rootCause}\n\nEvidence: ${h.evidence.join(', ') || 'None'}`).join('\n\n');
...
exports.MultiPassAgent = MultiPassAgent;
//# sourceMappingURL=MultiPassAgent.js.map