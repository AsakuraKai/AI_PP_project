...
exports.DocumentSynthesizer = void 0;
/**
...
class DocumentSynthesizer {
     ...
     */
    synthesize(rca, error, state, options = {}) {
        const { includeCodeContext = true, includeToolDetails = true, includeMetadata = true, maxCodeLength = 1000, } = options;
        const sections = [];
        // Title and summary
...
${steps}`;
     ...
     */
    generateCodeContextSection(rca, error, maxLength) {
...
${toolList}`;
     ...
     */
    generateMetadataSection(rca, error, state) {
        const metadata = [];
...
${metadata.join('  \n')}`;
     ...
     */
    generateQuickSummary(rca, error) {
        const errorType = this.formatErrorType(error.type);
        const confidence = Math.round(rca.confidence * 100);
        return `${errorType} in ${error.filePath}:${error.line} (${confidence}% confidence)`;
    }
}
exports.DocumentSynthesizer = DocumentSynthesizer;
//# sourceMappingURL=DocumentSynthesizer.js.map