...
class OutputValidator {
    constructor() {
        this.PASS_THRESHOLD = 0.60; // 60% quality required (PHASE 1 target)
    }
    /**
     * Validate RCA result quality with weighted dimensions
     */
    validate(result, _error) {
        const dimensions = {
     ...
     */
    checkFilePathSpecificity(result) {
        ...
        for (const path of filePaths) {
            ...
            const context = text.substring(Math.max(0, text.indexOf(path) - 50), Math.min(text.length, text.indexOf(path) + path.length + 50));
     ...
     */
    checkVersionSpecificity(result) {
        const text = result.rootCause + ' ' + result.fixGuidelines.join(' ');
        ...
        const hasGenericTerms = genericTerms.some(term => text.toLowerCase().includes(term));
     ...
     */
    buildFeedback(dimensions, issues) {
        ...
        for (const [dim, score] of weakDimensions) {
            feedback += `${dim}: ${(score * 100).toFixed(0)}%\n`;
     ...
     */
    checkVariableReferences(result) {
        const text = result.rootCause + ' ' + result.fixGuidelines.join(' ');
        // Generic terms (bad)
        const genericTerms = [
        ...
        const genericCount = genericTerms.filter(term => text.toLowerCase().includes(term)).length;
        ...
        const meaningfulRefs = specificRefs.filter(ref => ref.length > 3 && // Not too short
    ...
    checkVerificationSteps(result) {
        ...
        const found = verificationTerms.filter(term => text.toLowerCase().includes(term)).length;
        if (found >= 3) {
     ...
     */
    collectIssues(dimensions, _result) {
        const issues = [];
