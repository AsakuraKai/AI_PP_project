...
exports.ManifestAnalyzerTool = void 0;
class ManifestAnalyzerTool {
    /**
     * Parse manifest-related error from build output
     */
    parseManifestError(output) {
        return (this.parseManifestMergeConflict(output) ||
     ...
     */
    parseManifestMergeConflict(output) {
     ...
     */
    parseUndeclaredComponent(output) {
     ...
     */
    recommendPermissionFix(error) {
        if (error.type !== 'manifest_missing_permission' || !error.metadata) {
            return null;
        }
        const { requiredPermission, permissionName, isDangerous } = error.metadata;
...
if (ContextCompat.checkSelfPermission(this, Manifest.permission.${permissionName})
     ...
     */
    generateComponentDeclaration(error) {
        if (!error.type.startsWith('manifest_undeclared_') || !error.metadata) {
            return null;
        }
        const { componentType, componentClass } = error.metadata;
     ...
     */
    static isManifestError(output) {
        return (output.includes('AndroidManifest.xml') ||
...
exports.ManifestAnalyzerTool = ManifestAnalyzerTool;
// Common Android permissions and their categories
ManifestAnalyzerTool.DANGEROUS_PERMISSIONS = new Set([
    'CAMERA',
    'READ_CONTACTS',
    'WRITE_CONTACTS',
    'GET_ACCOUNTS',
...
//# sourceMappingURL=ManifestAnalyzerTool.js.map