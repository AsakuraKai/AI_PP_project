...
// ============================================================================
// NOTE: All UI-related global state has been removed
// Future UI implementation should create its own state management
// See docs/RCA_UI_WIRING_GUIDE.md for guidance
// ============================================================================
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  if (useNewUI) {
    ...
    // CHUNK 5: Initialize VirtualScrollProvider for large list optimization
    virtualScrollProvider = new VirtualScrollProvider({
      itemHeight: 32,      // Height of each tree item in pixels
      viewportHeight: 600, // Typical TreeView height
      overscan: 5,         // Render 5 items outside viewport for smooth scrolling
      estimatedItemCount: 100 // Estimated initial item count
    });
    log('info', 'VirtualScrollProvider initialized for optimized large list rendering');
    
    // Register Error Queue TreeView with virtual scrolling
    errorTreeProvider = new ErrorTreeProvider(context, errorQueueManager, virtualScrollProvider);
    context.subscriptions.push(
    ...
    // Register History TreeView with virtual scrolling
    historyTreeProvider = new HistoryTreeProvider(context, stateManager, virtualScrollProvider);
    context.subscriptions.push(
      vscode.window.registerTreeDataProvider(
        'rca-agent.history',
        historyTreeProvider
    ...
    if (analysisService && featureFlagManager?.isEnabled('batchAnalysis')) {
      ...
      log('info', 'Batch analysis commands registered');
    } else if (!featureFlagManager?.isEnabled('batchAnalysis')) {
      log('info', 'Batch analysis disabled by feature flag');
    }
    ...
    // Check if realtime detection should be enabled based on feature flag
    const realtimeEnabled = featureFlagManager?.isEnabled('experimentalFeatures') ?? true;
    realtimeDetector.setEnabled(realtimeEnabled);
    log('info', `Real-time error detection ${realtimeEnabled ? 'enabled' : 'disabled'} (experimental flag: ${realtimeEnabled})`);
    
    // Listen for feature flag changes to enable/disable realtime detection
    featureFlagManager?.onFlagChange((change) => {
      if (change.flag === 'experimentalFeatures') {
        realtimeDetector.setEnabled(change.value);
        log('info', `Real-time detection ${change.value ? 'enabled' : 'disabled'} via feature flag`);
      }
    });
  ...
  // Old toggle commands removed - educational mode and performance metrics now managed through panel settings
  ...
  log('info', 'RCA Agent UI fully initialized - panel, tree views, and inline integrations ready');
  vscode.window.showInformationMessage('RCA Agent activated! Look for the RCA icon in the Activity Bar.');
}
...
async function initializeChunk5Services(context: vscode.ExtensionContext): Promise<void> {
  try {
    ...
    // Register chat action commands with FixApplicationService integration
    if (fixApplicationService) {
      context.subscriptions.push(
        vscode.commands.registerCommand('rca-agent.applyFix', (result, context) => 
          applyFixCommand(result, fixApplicationService!, context)
        ),
      ...
      log('info', 'Chat action commands registered with FixApplicationService');
    } else {
      log('error', 'Cannot register chat action commands: FixApplicationService not initialized');
    }
