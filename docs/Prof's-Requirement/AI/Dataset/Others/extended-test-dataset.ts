...
fun updateUser(user: User) {
    ...
    {
        id: 'EXT-005',
        name: 'Sealed Class Missing Branch',
        category: 'kotlin',
        errorType: 'type_mismatch',
        complexity: 'easy',
        description: 'When expression missing sealed class branches - compile time error',
        errorText: `
e: WhenExpression.kt:34:5 'when' expression must be exhaustive, add necessary 'is Error' branch or 'else' branch instead`,
        expectedRootCause: 'When statement does not cover all sealed class branches',
        sampleCode: `sealed class Result {
    data class Success(val data: String) : Result()
