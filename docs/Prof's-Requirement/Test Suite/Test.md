...
- ⚠️ Warning: TypeScript 5.9.3 detected, @typescript-eslint/typescript-estree officially supports >=4.3.5 <5.4.0
...
# 4. Agent test (single case)
npx ts-node scripts/test-single-error.ts

# 5. ModelAdapter export
npx ts-node scripts/export-training-data.ts
Confirm: Run all 5 core tests? Or pick specific ones?